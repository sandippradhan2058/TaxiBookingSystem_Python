...
# username and password Label
                customer_user_ID_label = Label (self.root,text='Customer ID :- ',fg='Black',bg ='yellow',font=("Times New Roman",30,)).place(x=130,y=130)
                Password_label =Label (self.root,text='Password        :-',fg='Black',bg = 'yellow',font=("Times New Roman",30,)).place(x=130,y=210)

# Variables
                self.var_customer_user_ID = StringVar()
                self.var_Password = StringVar()
# Text box
                txt_customer_user_ID=Entry(self.root, textvariable=self.var_customer_user_ID,bg = 'yellow',font=("Times New ROman",34)).place(x=450,y=130)
                txt_Passowrd=Entry(self.root, textvariable=self.var_Password,show='*',bg ="yellow",font=("Times New ROman",34)).place(x=450,y=210)

#Login button
                Login_btn = Button(self.root,text="Login ",command = self.customer_login,font=("Times New Roman",40,),bg="black",fg="yellow",cursor='hand2', ).place(x=400,y=340,width=400,height=90)
        ...
        def customer_login(self):
                ...
                cur =con.cursor()
                try:
                        find_user = ("SELECT * from customer where customer_user_ID =? and Password =?")
                        cur.execute(find_user,[self.var_customer_user_ID.get(), self.var_Password.get()])
                        result = cur.fetchall()
                ...
                try:
                        ...
                        if self.var_customer_user_ID.get() == "" or self.var_Password.get()== "":
                                messagebox.showerror("Error"," You must enter the Customer ID and Passowrd",parent = self.root)
                                ...
                                messagebox.showerror("Error"," Please Input valid Customer ID or Password",parent = self.root)

                except Exception as ex:
                        messagebox.showerror("Error", "Error ddfggue to :",parent = self.root)
