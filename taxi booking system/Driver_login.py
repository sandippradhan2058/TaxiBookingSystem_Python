...
# username and password Label
                Driver_ID_label = Label (self.root,text='Driver ID :- ',fg='Black',bg ='yellow',font=("Times New Roman",30,)).place(x=130,y=130)
...
# Text box
                txt_driver_ID=Entry(self.root, textvariable=self.var_driver_ID,bg = 'yellow',font=("Times New ROman",34)).place(x=450,y=130)
                txt_Passowrd=Entry(self.root, textvariable=self.var_password_driver,show='*',bg ="yellow",font=("Times New ROman",34)).place(x=450,y=210)

#Login button
                Login_btn = Button(self.root,text="Login ",command = self.driver_login,font=("Times New Roman",40,),bg="black",fg="yellow",cursor='hand2', ).place(x=400,y=340,width=400,height=90)
        ...
        def driver_login(self):
                ...
                find_user = ("SELECT * from driver where driver_ID =? and password_driver =?")
                cur.execute(find_user,[self.var_driver_ID.get(), self.var_password_driver.get()])
                result = cur.fetchall()
                


                if self.var_driver_ID.get() == "" or self.var_password_driver.get()== "":
                        messagebox.showerror("Error"," You must enter the Driver ID and Passowrd",parent = self.root)
                        ...
                        messagebox.showerror("Error"," Please Input valid Driver ID or Password",parent = self.root)
